...
const MarsList = ({ cam, data }) => {
    ...
    return (
        <div className='marsList'>
            {
                data && cam == "cam1" &&
                data.cam1.map((img, index) => (
                    <img onClick={()=>openFullScreen(img)} className='min' src={img.img_src} alt="mars picture" key={index} />
                ))
            }
            {
                data && cam == "cam2" &&
                data.cam2.map((img,index) => (
                    <img onClick={()=>openFullScreen(img)} className='min' src={img.img_src} alt="mars picture" key={index} />
                ))
